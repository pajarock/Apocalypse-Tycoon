...
--[[
	...
	ConfiguraciÃ³n completa de todos los powerups del juego.

	TIPOS DE POWERUPS:
	- Basic: Escudo, Doble DaÃ±o, Full Heal, Velocidad
	- Movement: Dash mejorado
	- Defensive: Dron, Escudo de Base, Meteor Jammer, ExplosiÃ³n
...
-- -----------------------------------------------------------------------
-- ?? MÃQUINA EXPENDEDORA DE POWERUPS
-- -----------------------------------------------------------------------

PowerUpConfig.VendingMachine = {
	-- Costo inicial de la mÃ¡quina (comprar el edificio)
	PurchaseCost = 500,
	...
	-- Drop rates para la mÃ¡quina (mÃ¡s balanceados que waves)
	DropRates = {
		Common = 50,    -- 50%
		Uncommon = 35,  -- 35%
		Rare = 13,      -- 13%
...
PowerUpConfig.SpawnChances = {
	...
	Normal = {
		Common = 60,    -- 60% chance powerup comÃºn
		Uncommon = 30,  -- 30% chance powerup poco comÃºn
...
PowerUpConfig.PowerUps = {
	...
	GodShield = {
		...
		Rarity = "Common",
		Duration = {Min = 5, Max = 10}, -- DuraciÃ³n aleatoria entre 5-10 seg

		Icon = "???",
		Color = Color3.fromRGB(100, 200, 255), -- Azul/verde tÃ³xico
		...
		-- FunciÃ³n de activaciÃ³n (se llama en PowerUpModule)
		OnActivate = "ActivateGodShield",
	},

	-- ???????????????????????????????????????????????????????????????
	-- 2. DOUBLE DAMAGE (Doble DaÃ±o)
	-- ???????????????????????????????????????????????????????????????
	DoubleDamage = {
		Name = "Double Damage",
		Description = "10-15 seg de daÃ±o x2 (ideal para mini-boss)",
		Rarity = "Common",
	...
	-- ???????????????????????????????????????????????????????????????
	-- 3. DASH MEJORADO (EvasiÃ³n Perfecta)
	-- ???????????????????????????????????????????????????????????????
	SuperDash = {
		Name = "Super Dash",
	...
	-- ???????????????????????????????????????????????????????????????
	-- 4. FULL HEAL (CuraciÃ³n InstantÃ¡nea)
	-- ???????????????????????????????????????????????????????????????
	FullHeal = {
		Name = "Full Heal",
		Description = "Restaura 100% HP instantÃ¡neamente",
		Rarity = "Common",
	...
	-- ???????????????????????????????????????????????????????????????
	-- 5. MINI DRON (Defensa AutomÃ¡tica)
	-- ???????????????????????????????????????????????????????????????
	MiniDrone = {
		Name = "Mini Drone",
	...
	-- ???????????????????????????????????????????????????????????????
	-- 9. DEFENSIVE BURST (ExplosiÃ³n Defensiva)
	-- ???????????????????????????????????????????????????????????????
	DefensiveBurst = {
		Name = "Defensive Burst",
		Description = "ExplosiÃ³n que limpia meteoritos cercanos",
		Rarity = "Uncommon",
		Duration = 0, -- InstantÃ¡neo
	...
	CriticalParry = {
		Name = "Critical Parry",
		Description = "Convierte el prÃ³ximo impacto en explosiÃ³n que daÃ±a meteoritos cercanos",
		Rarity = "Rare",
		Duration = 0, -- Hasta que se use
...
-- Obtener powerup aleatorio segÃºn rarity
