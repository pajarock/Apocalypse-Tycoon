...
--[[
	...
	? GestiÃÂ³n de powerups activos por jugador
	? Spawning de powerups en el mundo
	? Sistema de pickup visual
	? Efectos VFX para cada powerup
	? IntegraciÃÂ³n con BaseModule y EconomyModule
...
-- Referencias a otros mÃÂ³dulos (se inyectarÃÂ¡n despuÃÂ©s)
...
export type ActivePowerUp = {
	...
	StartTime: number,
	Duration: number,
	Data: any?, -- Data especÃÂ­fica del powerup
}
...
-- Crear powerup fÃÂ­sico en el mundo
function PowerUpModule.SpawnPowerUpInWorld(powerUpId: string, position: Vector3): Model?
	...
	if not def then
		warn(("[PowerUpModule] PowerUp invÃÂ¡lido: %s"):format(powerUpId))
		return nil
	end
	...
	-- Base del powerup (parte fÃÂ­sica)
	...
	-- AnimaciÃÂ³n de flotaciÃÂ³n y rotaciÃÂ³n
	task.spawn(function()
		local startY = position.Y
		local t = 0
		while base and base.Parent do
			t += 0.016 -- ~60 FPS
			local offset = math.sin(t * 2) * 1 -- OscilaciÃÂ³n de 1 stud
			base.CFrame = CFrame.new(position.X, startY + offset, position.Z)
				* CFrame.Angles(0, t, 0) -- RotaciÃÂ³n continua
			task.wait(0.016)
		end
	end)
	...
	-- Detectar colisiÃÂ³n con jugador
	...
	-- Auto-destruir despuÃÂ©s de 30 segundos
...
-- -----------------------------------------------------------------------
-- SISTEMA DE COLECCIÃÂN Y ACTIVACIÃÂN
-- -----------------------------------------------------------------------
...
function PowerUpModule.ActivatePowerUp(userId: number, powerUpId: string)
	...
	-- Llamar funciÃÂ³n de activaciÃÂ³n especÃÂ­fica
	...
	-- Programar expiraciÃÂ³n
	if duration > 0 then
		task.delay(duration, function()
			PowerUpModule.ExpirePowerUp(userId, powerUpId)
		end)
	else
		-- ? FIX: Powerups instantÃÂ¡neos (duration = 0) deben expirar inmediatamente
	...
	if DEBUG then
		print(("[PowerUpModule] PowerUp activado: %s para userId %d (duraciÃÂ³n: %.1fs)"):format(
...
function PowerUpModule.ExpirePowerUp(userId: number, powerUpId: string)
	...
	if not found and DEBUG then
		warn(("[PowerUpModule] ?? IntentÃÂ³ expirar powerup inexistente: %s para userId %d"):format(powerUpId, userId))
	end

	-- Llamar funciÃÂ³n de limpieza especÃÂ­fica
	local def = PowerUpConfig.PowerUps[powerUpId]
	...
	-- Cleanup especÃÂ­fico por powerup (asegurar limpieza completa)
...
-- -----------------------------------------------------------------------
-- IMPLEMENTACIÃÂN DE POWERUPS ESPECÃÂFICOS
-- -----------------------------------------------------------------------
...
function PowerUpModule.ActivateDoubleDamage(userId: number, powerUp: ActivePowerUp)
	...
	-- AquÃÂ­ solo guardamos que estÃÂ¡ activo
...
function PowerUpModule.ActivateMiniDrone(userId: number, powerUp: ActivePowerUp)
	...
	-- Auto-destruir despuÃÂ©s de la duraciÃÂ³n
...
function PowerUpModule.ActivateSpeedBoost(userId: number, powerUp: ActivePowerUp)
	...
	-- Restaurar despuÃÂ©s de la duraciÃÂ³n
	task.delay(powerUp.Duration, function()
...
-- -----------------------------------------------------------------------
-- GETTERS PARA OTROS MÃÂDULOS
-- -----------------------------------------------------------------------

-- Verificar si SuperDash estÃÂ¡ activo (para sistema de dash)
...
-- Verificar si BaseShield puede bloquear daÃÂ±o
...
function PowerUpModule.TryUseCriticalParry(userId: number, meteorPosition: Vector3): boolean
	if CriticalParryActive[userId] then
		...
		-- Crear explosiÃÂ³n en la posiciÃÂ³n del meteorito
		local explosion = Instance.new("Explosion")
...
-- Verificar si Meteor Jammer estÃÂ¡ activo
...
function PowerUpModule.TryDroneIntercept(userId: number): boolean
	...
	for _, powerUp in ipairs(PlayerPowerUps[userId].ActivePowerUps) do
		if powerUp.PowerUpId == "MiniDrone" and powerUp.Data.InterceptsRemaining and powerUp.Data.InterceptsRemaining > 0 then
			...
			if DEBUG then
				print(("[PowerUpModule] Dron interceptÃÂ³ meteorito para userId %d (quedan %d)"):format(
...
-- -----------------------------------------------------------------------
-- SISTEMA DE SPAWN AUTOMÃÂTICO
-- -----------------------------------------------------------------------

-- Spawnear powerup para un jugador que completÃÂ³ una wave
function PowerUpModule.SpawnPowerUpForPlayer(userId: any, waveType: string, waveNumber: number?)
	...
	if type(userId) ~= "number" then
		warn(("[PowerUpModule] ? SpawnPowerUpForPlayer recibiÃÂ³ userId invÃÂ¡lido: %s (tipo: %s)"):format(
			tostring(userId), type(userId)
			))
		warn("[PowerUpModule] Stack trace - Esto se llamÃÂ³ desde EventManager probablemente")
		warn("[PowerUpModule] ?? SOLUCIÃÂN: En EventManager.lua lÃÂ­nea ~349, cambia a:")
	...
	-- ? SISTEMA HÃÂBRIDO: DeterminÃÂ­stico + ProbabilÃÂ­stico
	local powerUpId: string? = nil
	local wave = waveNumber or 0

	-- WAVE 3: Siempre FullHeal (introducciÃÂ³n amigable)
	if wave == 3 then
		powerUpId = "FullHeal"
		if DEBUG then
	...
	elseif waveType == "Boss" then
		...
		if DEBUG then
			print(("[PowerUpModule] ?? Boss Wave %d - Spawneando %s ÃÂ©pico"):format(wave, powerUpId))
		end
	...
	elseif waveType == "MiniBoss" then
		...
		-- OTRAS WAVES: Random segÃÂºn probabilidad de waveType
	else
...
--[[
	...
	@return boolean - true si se agregÃÂ³ exitosamente
...
function PowerUpModule.AddToInventory(userId: number, powerUpId: string): (boolean, string?)
	...
	if #inventory >= MAX_INVENTORY_SLOTS then
		return false, "Inventario lleno! (mÃÂ¡x 5)"
	end
...
--[[
	...
	@param userId number
	@param slotIndex number - ÃÂ­ndice del slot (1-based)
	@return boolean - true si se usÃÂ³ exitosamente
]]
...
-- -----------------------------------------------------------------------
-- ?? SISTEMA DE MÃÂQUINA EXPENDEDORA
-- -----------------------------------------------------------------------
...
--[[
	Comprar powerup desde la mÃÂ¡quina expendedora.

	@param userId number - ID del jugador
	@return boolean - true si la compra fue exitosa
	@return string? - Mensaje de error si fallÃÂ³
]]
function PowerUpModule.PurchaseFromVendingMachine(userId: number): (boolean, string?)
	...
	if not VendingMachineFirstUse[userId] then
		...
		if DEBUG then
			print(("[PowerUpModule] ?? Primer uso de mÃÂ¡quina - IncomeBoost gratis garantizado para userId %d"):format(userId))
		end
	else
		...
		if DEBUG then
			print(("[PowerUpModule] ?? MÃÂ¡quina expendedora - Rareza: %s, PowerUp: %s, Precio: $%d"):format(
	...
	if not success then
		-- Revertir el cobro si el inventario estÃÂ¡ lleno
		if EconomyModule then
			local state = EconomyModule.GetState(userId)
			if state then
				state.Cash += price
	...
	if DEBUG then
		print(("[PowerUpModule] ? Compra exitosa - userId %d recibiÃÂ³ %s por $%d (agregado a inventario)"):format(
...
-- Obtener rareza random para mÃÂ¡quina expendedora
...
--[[
	Spawnear chicle/pelota fÃÂ­sica de powerup que puede ser recogido.

	@param powerUpId string - ID del powerup
	@param position Vector3 - PosiciÃÂ³n donde spawnearlo
	@param userId number - ID del dueÃÂ±o (para que solo ÃÂ©l lo pueda recoger)
...
function PowerUpModule.SpawnGumball(powerUpId: string, position: Vector3, userId: number): Part?
	...
	-- Sparkles para efecto mÃÂ¡gico
	...
	gumball.Touched:Connect(function(hit)
		...
		if not player or player.UserId ~= userId then return end -- Solo el dueÃÂ±o puede recogerlo

		-- Ya estÃÂ¡ en el inventario (se agregÃÂ³ al comprar)
		-- Solo destruir el visual
		gumball:Destroy()

		if DEBUG then
			print(("[PowerUpModule] ?? Jugador %d recogiÃÂ³ chicle de %s"):format(userId, powerUpId))
		end
	end)

	-- Auto-destruir despuÃÂ©s de 60 segundos si no se recoge
...
-- -----------------------------------------------------------------------
-- SISTEMA DE INICIALIZACIÃÂN Y CLEANUP
-- -----------------------------------------------------------------------
