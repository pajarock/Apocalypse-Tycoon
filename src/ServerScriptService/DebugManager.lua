...
--[[
	...
	- B: Spawn Boss instantÃÂ¡neo
	- V: Spawn Mini-Boss instantÃÂ¡neo
	- C: AÃÂ±adir $10,000
	- Shift+C: AÃÂ±adir $100,000
	- Y: Reset a Wave 1
	- G: Toggle Invencibilidad (God Mode)
	- 1-9: Saltar a wave especÃÂ­fico (10, 20, 30, etc.)
...
-- Esperar a que Economy y DataStore estÃÂ©n cargados
task.wait(2)
...
DebugSkipWave.OnServerEvent:Connect(function(player: Player, skipAmount: number?)
	...
	print(string.format("[DEBUG] %s saltÃÂ³ al wave %d%s", player.Name, newWave, bossType and (" (" .. bossType .. ")") or ""))
end)

-- JUMP TO WAVE: Salta a un wave especÃÂ­fico
DebugJumpToWave.OnServerEvent:Connect(function(player: Player, targetWave: number)
	if not checkCooldown(player.UserId) then return end

	if not targetWave or targetWave < 1 then
		notify(player, "? Wave invÃÂ¡lido")
		return
	end
	...
	print(string.format("[DEBUG] %s saltÃÂ³ al wave %d%s", player.Name, targetWave, bossType and (" (" .. bossType .. ")") or ""))
end)
...
DebugSpawnBoss.OnServerEvent:Connect(function(player: Player)
	...
	-- Usar el wave actual o un mÃÂºltiplo de 10
	...
	print(string.format("[DEBUG] %s spawneÃÂ³ un BOSS (wave %d)", player.Name, waveNum))
end)
...
DebugSpawnMiniBoss.OnServerEvent:Connect(function(player: Player)
	...
	-- Usar el wave actual o un mÃÂºltiplo de 5
	local waveNum = CurrentWaveValue.Value
	if waveNum % 5 ~= 0 then
	...
	print(string.format("[DEBUG] %s spawneÃÂ³ un MINI-BOSS (wave %d)", player.Name, waveNum))
end)

-- ADD CASH: AÃÂ±ade dinero al jugador
DebugAddCash.OnServerEvent:Connect(function(player: Player, amount: number?)
	...
	if success then
		-- Formatear el nÃÂºmero para mostrar (simple)
		...
		print(string.format("[DEBUG] %s aÃÂ±adiÃÂ³ $%d", player.Name, cashToAdd))
	else
		notify(player, "? Error aÃÂ±adiendo dinero: " .. tostring(errorMsg))
		warn(string.format("[DEBUG] Error aÃÂ±adiendo cash a %s: %s", player.Name, tostring(errorMsg)))
	end
end)
...
DebugResetWave.OnServerEvent:Connect(function(player: Player)
	...
	print(string.format("[DEBUG] %s reseteÃÂ³ el wave a 1", player.Name))
end)
...
DebugToggleInvincibility.OnServerEvent:Connect(function(player: Player)
	...
	print(string.format("[DEBUG] %s %s la invencibilidad",
		player.Name,
		newInvincible and "activÃÂ³" or "desactivÃÂ³"))
end)
