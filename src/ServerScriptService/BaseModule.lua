...
--[[
	...
	1. Eliminada dependencia circular con EventManager (l�nea 28 removida)
	2. Eliminado c�digo de shake duplicado (l�neas 272-276 removidas)
	3. El shake ahora se maneja completamente en EventManager

	CAMBIOS RESPECTO A LA VERSI�N ANTERIOR:
	- L�nea 28: ELIMINADA - No require EventManager
	- L�neas 271-276: ELIMINADAS - No llama a EventManager.ShakePlayer
	...
	? Da�o con reducci�n por shields
...
function BaseModule.ApplyDamage(userId: number, rawDamage: number): number
	...
	if state.IsInvulnerable then
		...
		-- ? NUEVO: Notificar al cliente que evadi� da�o exitosamente
		...
		if player then
			...
			if Remotes then
				...
				if DashEvaded and DashEvaded:IsA("RemoteEvent") then
					...
					if DEBUG then
						print(("[BaseModule] ??? EVADIDO! userId %d evadi� %d de da�o"):format(userId, rawDamage))
					end
				end
			end
		end
	...
	-- ? POWERUP: Mini Drone - Verificar si puede interceptar el da�o
	local pum = getPowerUpModule()
	if pum and pum.TryDroneIntercept and pum.TryDroneIntercept(userId) then
		state.LastDamageTime = now
		if DEBUG then
			print(("[BaseModule] ?? DRON INTERCEPT�! userId %d - da�o bloqueado: %d"):format(userId, rawDamage))
		end
		return 0
	end

	-- ? POWERUP: Base Shield - Verificar si puede bloquear el da�o
	if pum and pum.TryUseBaseShield and pum.TryUseBaseShield(userId) then
		state.LastDamageTime = now
		if DEBUG then
			print(("[BaseModule] ??? BASE SHIELD! userId %d - da�o bloqueado: %d"):format(userId, rawDamage))
		end
		return 0
	end

	-- ? POWERUP: Critical Parry - Marcar posici�n del meteorito para parry
	local meteorPosition = nil
	-- La posici�n del meteorito deber�a pasarse como par�metro, por ahora usamos nil
	-- Esto se manejar� cuando el meteorito impacte

	-- Reducci�n por escudo
	...
	-- ? ARREGLADO: Eliminado c�digo de shake - ahora se maneja en EventManager
...
-- ? ARREGLADO: Removidas declaraciones duplicadas - usar las de l�nea 79-80
...
function BaseModule.OnBaseDead(userId: number)
	...
	-- ? MARCAR QUE MURI� DURANTE ESTA WAVE
	if BaseState[userId] then
	...
	-- 2?? CALCULAR PENALIZACI�N (25% del dinero actual)
	local moneyLost = math.floor(currentMoney * 0.25)
	local newAmount = math.max(0, currentMoney - moneyLost)

	if DEBUG then
		print(("[BaseModule] ?? P�rdida calculada: $%d (25%%)"):format(moneyLost))
	...
	-- 4?? ACTIVAR GUARDIAN (protecci�n por 120 segundos)
	...
	ActiveGuardianTasks[userId] = task.spawn(function()
		...
		while tick() - startTime < forceDuration do
			...
			if not protection then
				if DEBUG then
					print(("[BaseModule] ?? Guardian: Protecci�n eliminada externamente"):format())
				end
				break
			end
			...
			-- ? ARREGLADO: Solo evitar que BAJE del m�nimo, permitir que SUBA
			if cash.Value < targetAmount then
				local oldValue = cash.Value

				-- Forzar al m�nimo solo si baj�
				...
				if DEBUG then
					print(("[BaseModule] ??? Guardian PROTEGI�: Dinero baj� de $%d a $%d"):format(
						targetAmount, oldValue
						))
					print(("[BaseModule] ??? Guardian RESTAUR�: $%d ? $%d"):format(
						oldValue, targetAmount
						))
				end
			elseif DEBUG and cash.Value > targetAmount then
				-- Income normal funcionando, no hacer nada
				print(("[BaseModule] ? Guardian: Income OK ($%d, m�nimo protegido: $%d)"):format(
					cash.Value, targetAmount
					))
			end
		end
		...
		ActiveGuardianTasks[userId] = nil
		if DEBUG then
			print(("[BaseModule] ? Guardian TERMINADO - Protecci�n finalizada"):format())
		end
	end)
	...
	-- Actualizar visuales (si BaseVisualsManager est� disponible)
...
function BaseModule.SetShieldLevel(userId: number, level: number)
	...
	if DEBUG then
		print(("[BaseModule] Shield level userId %d: %d (reducci�n: %.0f%%)"):format(
...
-- ? NUEVA FUNCI�N: Actualizar Guardian cuando se hace compra/reparaci�n leg�tima
function BaseModule.UpdateGuardianTarget(userId: number, newAmount: number)
	...
	-- Actualizar el target amount para que el Guardian no devuelva dinero en compras leg�timas
...
function BaseModule.IncrementMeteorsSurvived(userId: number)
	...
	if DEBUG and BaseState[userId].MeteorsSurvived % 10 == 0 then
		print(("[BaseModule] Usuario %d sobrevivi� %d meteoritos!"):format(
...
-- ? Verificar si muri� durante la wave actual
...
-- Loop de regeneraci�n (iniciar en el server)
task.spawn(function()
	while true do
		task.wait(1)
...
if DEBUG then
	print("[BaseModule ARREGLADO] ? M�dulo cargado (sin dependencia circular)")
end
			...
			if data and type(data) == "table" then
				...
				if player then
					...
					if cashValue and data.targetAmount then
						-- Si el dinero est� MAYOR al target, forzar reducci�n
						...
						-- Despu�s de 120 segundos (2 autosaves), dejar de forzar
...
-------------------------------------------------------------------------
-- ? ACTUALIZAR GUARDIAN DESPU�S DE COMPRAS LEG�TIMAS
-------------------------------------------------------------------------
...
function BaseModule.UpdateGuardianTarget(userId: number, newTarget: number)
	...
	if not protection then
		if ActiveGuardianTasks[userId] then
			...
			if DEBUG then
				print(("[BaseModule] ??? Guardian: Creada protecci�n para compra leg�tima - Target: $%d"):format(newTarget))
			end
		else
			if DEBUG then
	...
	if DEBUG then
		print(("[BaseModule] ??? Guardian: Target actualizado $%d ? $%d para userId %d (compra leg�tima)"):format(
