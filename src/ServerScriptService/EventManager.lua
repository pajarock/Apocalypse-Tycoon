...
--[[
	...
	2. ValidaciÃÂ³n de BaseModule antes de usar sus funciones
	...
	- LÃÂ­nea 56: BaseModule = nil (se inyectarÃÂ¡)
	- LÃÂ­nea 67-72: Nueva funciÃÂ³n SetBaseModule()
	- LÃÂ­nea 293-310: ValidaciÃÂ³n de BaseModule aÃÂ±adida
	- LÃÂ­nea 280-283: Flash rojo agregado en impactos
--]]
...
if not CameraShake then
	...
	warn("[EventManager] RemoteEvent 'CameraShake' creado automÃÂ¡ticamente")
end

-- ?? NUEVO: Remote para notificaciones (dodge, daÃÂ±o, etc.)
...
if not ShowNotification then
	...
	warn("[EventManager] RemoteEvent 'ShowNotification' creado automÃÂ¡ticamente")
end
...
if not ScreenFlash then
	...
	warn("[EventManager] RemoteEvent 'ScreenFlash' creado automÃÂ¡ticamente")
end
...
if not BaseDead then
	...
	warn("[EventManager] RemoteEvent 'BaseDead' creado automÃÂ¡ticamente")
end


local Config     = require(game.ServerStorage.Config.Config)

-- ? ARREGLADO: BaseModule se inyectarÃÂ¡ desde Main.Server.lua
...
-------------------------------------------------------------------------
-- ? NUEVA FUNCIÃÂN: Inyectar BaseModule desde Main.Server.lua
-------------------------------------------------------------------------
...
local function createMeteor(meteorType: string): Part
	...
	-- ?? NUEVO: Usar VFX trail en lugar de Fire bÃÂ¡sico
	-- El trail se spawnearÃÂ¡ con VFXManager (ver abajo)

	-- Mantener efectos bÃÂ¡sicos para compatibilidad
...
local function spawnMeteorTowards(plr: Player, startPos: Vector3, targetPos: Vector3, meteorType: string?)
	...
	task.delay(Config.METEOR_LIFETIME, function()
		if meteor and meteor.Parent and not applied then
			if Config.DEBUG_MODE then
				print("[Meteor] Timeout sin impacto vÃÂ¡lido")
			end
			cleanup()
		end
	end)
	...
	conn = meteor.Touched:Connect(function(hit: BasePart)
		...
		-- ?? NUEVO: ExplosiÃÂ³n visual ÃÂ©pica
		...
		-- ?? NUEVO: DAÃÂO A JUGADORES (mecÃÂ¡nica de salto para evadir)
		...
		-- ? ARREGLADO: LÃÂ³gica de daÃÂ±o a la base con validaciÃÂ³n de BaseModule
		...
		if BaseModule and BaseModule.ApplyDamage then
			...
			if BaseModule.GetHP and BaseModule.GetHP(userId) > 0 then
				print(("[METEOR] Jugador %d sobreviviÃÂ³!"):format(userId))
			end
		else
			warn("[EventManager] ?? BaseModule no estÃÂ¡ disponible - no se aplicÃÂ³ daÃÂ±o")
			warn("[EventManager] ?? AsegÃÂºrate de llamar EventManager:SetBaseModule(Base) en Main.Server.lua")
		end
		...
		if ownerPlr and appliedAmount > 0 then
			...
			-- ?? NUEVO: VFX de daÃÂ±o en la base
			local basePart = getPlayerBasePart(ownerPlr)
			if basePart then
			...
			if BaseModule and BaseModule.IsLowHP and BaseModule.IsLowHP(userId) then
				notifyPlayer(ownerPlr, "?? HP CRÃÂTICO!", Config.UI_COLORS.Warning)
			end
		end
...
local function spawnMeteorOverBase(plr: Player, height: number?, spread: number?, meteorType: string?)
	...
	if not base then
		warn(("[Meteor] No encontrÃÂ© base del jugador: %s"):format(plr.Name))
		return
	end
...
-- Evento: MeteorStorm (sin cambios de lÃÂ³gica, solo VFX)
...
-- Fase 2: "CÃÂ­rculo Infernal" - 6 meteoritos Normal en cÃÂ­rculo rotando
local function BossPhase_Circle(plr: Player)
	local base = getPlayerBasePart(plr)
	if not base then return end

	local basePos = base.Position
	local radius = 35  -- ? Radio aumentado para patrÃÂ³n mÃÂ¡s visible
	local meteorCount = 6
	...
	for i = 1, meteorCount do
		task.delay((i - 1) * 0.5, function() -- 0.5s entre cada meteorito
			...
			-- ? Spawn desde arriba del punto del cÃÂ­rculo
			local startPos = basePos + offset + Vector3.new(0, 100, 0)
			-- ? ARREGLADO: Caer directamente en el punto del cÃÂ­rculo (offset completo)
	...
	if Config.DEBUG_MODE then
		print(("[BossMeteor] Fase 2 'CÃÂ­rculo Infernal' ejecutada para %s"):format(plr.Name))
	end
end

-- Fase 3: "El Coloso" - 1 meteorito GIGANTE lento (8s caÃÂ­da)
local function BossPhase_Colossus(plr: Player)
	...
	-- Spawn muy alto para caÃÂ­da lenta ÃÂ©pica
	local startPos = basePos + Vector3.new(0, 250, 0)
	local targetPos = basePos

	-- Usar tipo Boss (mÃÂ¡s grande, mÃÂ¡s lento, mÃÂ¡s daÃÂ±o)
	...
	-- Velocidad MUY lenta para caÃÂ­da dramÃÂ¡tica
	local dir = (targetPos - startPos).Unit
	local bv = Instance.new("BodyVelocity")
	bv.MaxForce = Vector3.new(1e6, 1e6, 1e6)
	...
	-- NotificaciÃÂ³n ÃÂ©pica
	if ShowNotification then
		ShowNotification:FireClient(plr, "?? THE COLOSSUS DESCENDS!", 4)
	end

	-- Sistema de impacto (igual que spawnMeteorTowards pero con mÃÂ¡s drama)
	local applied = false
	local conn: RBXScriptConnection? = nil
	...
	task.delay(12, function() -- MÃÂ¡s tiempo para caÃÂ­da lenta
		if meteor and meteor.Parent and not applied then
			cleanup()
		end
	end)
	...
	conn = meteor.Touched:Connect(function(hit: BasePart)
		...
		if BaseModule and BaseModule.ApplyDamage then
			...
			if Config.DEBUG_MODE then
				print(("[BossMeteor] COLOSSUS impactÃÂ³ - daÃÂ±o=%d"):format(rawDamage))
			end
...
-- Fase 4: "TriÃÂ¡ngulo de Fuego" - 3 meteoritos Large en triÃÂ¡ngulo simultÃÂ¡neo
local function BossPhase_Triangle(plr: Player)
	...
	-- TriÃÂ¡ngulo equilÃÂ¡tero: 3 puntos a 120ÃÂ° cada uno
	for i = 1, 3 do
		task.delay(0.2, function() -- Casi simultÃÂ¡neos (0.2s delay para drama)
			local angle = (i / 3) * math.pi * 2 + math.pi / 6 -- Offset para rotaciÃÂ³n
			local offset = Vector3.new(
				math.cos(angle) * radius,
				0,
				math.sin(angle) * radius
	...
	if Config.DEBUG_MODE then
		print(("[BossMeteor] Fase 4 'TriÃÂ¡ngulo de Fuego' ejecutada para %s"):format(plr.Name))
	end
end

-- ?? FUNCIÃÂN PRINCIPAL: BossMeteor
function EventManager:BossMeteor(waveNum: number, isFullBoss: boolean)
	...
	for idx, phaseNum in ipairs(phasesToExecute) do
		...
		-- Timing entre fases
		if phaseNum == 3 then
			totalDuration += 10 -- Coloso tarda mÃÂ¡s
		else
...
if Config.DEBUG_MODE then
	print("[EVENTMANAGER ARREGLADO] ? MÃÂ³dulo cargado - esperando inyecciÃÂ³n de BaseModule")
end
