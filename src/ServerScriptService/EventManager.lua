...
--[[
	...
	? ARREGLOS EN ESTA VERSIÃN:
	1. BaseModule ahora se inyecta desde Main.Server.lua (no require directo)
	2. ValidaciÃ³n de BaseModule antes de usar sus funciones
	...
	- LÃ­nea 56: BaseModule = nil (se inyectarÃ¡)
	- LÃ­nea 67-72: Nueva funciÃ³n SetBaseModule()
	- LÃ­nea 293-310: ValidaciÃ³n de BaseModule aÃ±adida
	- LÃ­nea 280-283: Flash rojo agregado en impactos
--]]
...
if not CameraShake then
	...
	warn("[EventManager] RemoteEvent 'CameraShake' creado automÃ¡ticamente")
end

-- ?? NUEVO: Remote para notificaciones (dodge, daÃ±o, etc.)
...
if not ShowNotification then
	...
	warn("[EventManager] RemoteEvent 'ShowNotification' creado automÃ¡ticamente")
end
...
if not ScreenFlash then
	...
	warn("[EventManager] RemoteEvent 'ScreenFlash' creado automÃ¡ticamente")
end
...
if not BaseDead then
	...
	warn("[EventManager] RemoteEvent 'BaseDead' creado automÃ¡ticamente")
end


local Config     = require(game.ServerStorage.Config.Config)

-- ? ARREGLADO: BaseModule se inyectarÃ¡ desde Main.Server.lua
...
-------------------------------------------------------------------------
-- ? NUEVA FUNCIÃN: Inyectar BaseModule desde Main.Server.lua
-------------------------------------------------------------------------
...
local function createMeteor(meteorType: string): Part
	...
	-- ?? NUEVO: Usar VFX trail en lugar de Fire bÃ¡sico
	-- El trail se spawnearÃ¡ con VFXManager (ver abajo)

	-- Mantener efectos bÃ¡sicos para compatibilidad
...
local function spawnMeteorTowards(plr: Player, startPos: Vector3, targetPos: Vector3, meteorType: string?)
	...
	task.delay(Config.METEOR_LIFETIME, function()
		if meteor and meteor.Parent and not applied then
			if Config.DEBUG_MODE then
				print("[Meteor] Timeout sin impacto vÃ¡lido")
			end
			cleanup()
		end
	end)
	...
	conn = meteor.Touched:Connect(function(hit: BasePart)
		...
		-- ?? NUEVO: ExplosiÃ³n visual Ã©pica
		...
		-- ?? NUEVO: DAÃO A JUGADORES (mecÃ¡nica de salto para evadir)
		...
		-- ? ARREGLADO: LÃ³gica de daÃ±o a la base con validaciÃ³n de BaseModule
		...
		if BaseModule and BaseModule.ApplyDamage then
			...
			if BaseModule.GetHP and BaseModule.GetHP(userId) > 0 then
				print(("[METEOR] Jugador %d sobreviviÃ³!"):format(userId))
			end
		else
			warn("[EventManager] ?? BaseModule no estÃ¡ disponible - no se aplicÃ³ daÃ±o")
			warn("[EventManager] ?? AsegÃºrate de llamar EventManager:SetBaseModule(Base) en Main.Server.lua")
		end
		...
		if ownerPlr and appliedAmount > 0 then
			...
			if BaseModule and BaseModule.IsLowHP and BaseModule.IsLowHP(userId) then
				notifyPlayer(ownerPlr, "?? HP CRÃTICO!", Config.UI_COLORS.Warning)
			end
		end
...
local function spawnMeteorOverBase(plr: Player, height: number?, spread: number?, meteorType: string?)
	...
	if not base then
		warn(("[Meteor] No encontrÃ© base del jugador: %s"):format(plr.Name))
		return
	end
...
-- Evento: MeteorStorm (sin cambios de lÃ³gica, solo VFX)
...
-- Fase 2: "CÃ­rculo Infernal" - 6 meteoritos Normal en cÃ­rculo rotando
local function BossPhase_Circle(plr: Player)
	local base = getPlayerBasePart(plr)
	if not base then return end

	local basePos = base.Position
	local radius = 35  -- ? Radio aumentado para patrÃ³n mÃ¡s visible
	local meteorCount = 6
	...
	for i = 1, meteorCount do
		task.delay((i - 1) * 0.5, function() -- 0.5s entre cada meteorito
			...
			-- ? Spawn desde arriba del punto del cÃ­rculo
			local startPos = basePos + offset + Vector3.new(0, 100, 0)
			-- ? ARREGLADO: Caer directamente en el punto del cÃ­rculo (offset completo)
	...
	if Config.DEBUG_MODE then
		print(("[BossMeteor] Fase 2 'CÃ­rculo Infernal' ejecutada para %s"):format(plr.Name))
	end
end

-- Fase 3: "El Coloso" - 1 meteorito GIGANTE lento (8s caÃ­da)
local function BossPhase_Colossus(plr: Player)
	...
	-- Spawn muy alto para caÃ­da lenta Ã©pica
	local startPos = basePos + Vector3.new(0, 250, 0)
	local targetPos = basePos

	-- Usar tipo Boss (mÃ¡s grande, mÃ¡s lento, mÃ¡s daÃ±o)
	...
	-- Velocidad MUY lenta para caÃ­da dramÃ¡tica
	local dir = (targetPos - startPos).Unit
	local bv = Instance.new("BodyVelocity")
	bv.MaxForce = Vector3.new(1e6, 1e6, 1e6)
	...
	-- NotificaciÃ³n Ã©pica
	if ShowNotification then
		ShowNotification:FireClient(plr, "?? THE COLOSSUS DESCENDS!", 4)
	end

	-- Sistema de impacto (igual que spawnMeteorTowards pero con mÃ¡s drama)
	local applied = false
	local conn: RBXScriptConnection? = nil
	...
	task.delay(12, function() -- MÃ¡s tiempo para caÃ­da lenta
		if meteor and meteor.Parent and not applied then
			cleanup()
		end
	end)
	...
	conn = meteor.Touched:Connect(function(hit: BasePart)
		...
		if BaseModule and BaseModule.ApplyDamage then
			...
			if Config.DEBUG_MODE then
				print(("[BossMeteor] COLOSSUS impactÃ³ - daÃ±o=%d"):format(rawDamage))
			end
...
-- Fase 4: "TriÃ¡ngulo de Fuego" - 3 meteoritos Large en triÃ¡ngulo simultÃ¡neo
local function BossPhase_Triangle(plr: Player)
	...
	-- TriÃ¡ngulo equilÃ¡tero: 3 puntos a 120Â° cada uno
	for i = 1, 3 do
		task.delay(0.2, function() -- Casi simultÃ¡neos (0.2s delay para drama)
			local angle = (i / 3) * math.pi * 2 + math.pi / 6 -- Offset para rotaciÃ³n
			local offset = Vector3.new(
				math.cos(angle) * radius,
				0,
				math.sin(angle) * radius
	...
	if Config.DEBUG_MODE then
		print(("[BossMeteor] Fase 4 'TriÃ¡ngulo de Fuego' ejecutada para %s"):format(plr.Name))
	end
end

-- ?? FUNCIÃN PRINCIPAL: BossMeteor
function EventManager:BossMeteor(waveNum: number, isFullBoss: boolean)
	...
	for idx, phaseNum in ipairs(phasesToExecute) do
		...
		-- Timing entre fases
		if phaseNum == 3 then
			totalDuration += 10 -- Coloso tarda mÃ¡s
		else
...
if Config.DEBUG_MODE then
	print("[EVENTMANAGER ARREGLADO] ? MÃ³dulo cargado - esperando inyecciÃ³n de BaseModule")
end
