...
--[[
	...
	-----------------------------------------------------------------------
	VERSIÃN: 5.0 PRODUCTION READY
	AUTOR: Tu Equipo
	ÃLTIMA ACTUALIZACIÃN: 2025
	
	CARACTERÃSTICAS:
	? Sistema completo de economÃ­a con anti-exploit
	? Sistema de waves dinÃ¡micas con boss fights
	? Sistema de achievements y badges
	? Sistema de prestige y progresiÃ³n
	? Sistema VIP y gamepasses
	? Sistema de daily rewards
	? Sistema de leaderboards global
	? Sistema de commands (admin y debug)
	? Sistema de analytics y telemetrÃ­a
	...
	- Testear en servidor privado antes de pÃºblico
...
-- MÃ³dulo de achievements (crear si no existe)
...
-- IDs de gamepasses (CONFIGURAR EN PRODUCCIÃN)
...
-- ?? Remote para comprar powerups desde la mÃ¡quina
...
if not PurchasePowerUp then
	...
	print("[POWERUP] RemoteEvent 'PurchasePowerUp' creado automÃ¡ticamente")
end
...
if not UseFromInventory then
	...
	print("[POWERUP] RemoteEvent 'UseFromInventory' creado automÃ¡ticamente")
end
...
local WaveResult = getOrCreateRemote("WaveResult", "Event") :: RemoteEvent -- ? Para mensajes Ã©picos
...
local function checkRateLimit(userId: number, action: string): boolean
	...
	if data.count >= limit.max then
		if DEBUG then
			warn(("[RATE_LIMIT] Usuario %d excediÃ³ lÃ­mite de %s"):format(userId, action))
		end
		return false
	end
...
local function logAnalytic(eventName: string, data: any?)
	...
	-- AquÃ­ puedes integrar con servicios externos como Google Analytics
	-- o simplemente guardar en DataStore para anÃ¡lisis posterior
...
local function createLeaderstats(plr: Player)
	...
	-- Stats adicionales (ocultas para UI pero Ãºtiles)
...
local function createBaseBillboard(plate: BasePart, playerName: string, userId: number)
	...
	-- Actualizar HP label periÃ³dicamente
	task.spawn(function()
...
local function assignBase(plr: Player, slot: number)
	...
	-- ?? Spawnear mÃ¡quina expendedora de powerups
	...
	if vendingMachine then
		...
		-- La mÃ¡quina tiene ~7 studs de altura, asÃ­ que Y=4 la pone parcialmente visible
		...
		if DEBUG then
			print(("[POWERUP] ?? MÃ¡quina expendedora spawneada para %s en posiciÃ³n %s"):format(plr.Name, tostring(machinePos)))
		end
	end
...
-------------------------------------------------------------------------
-- SISTEMA DE CATEGORÃAS Y PLACEMENT
-------------------------------------------------------------------------
...
local function getNextBuildCFrame(basePart: BasePart, index: number, category: string?): CFrame?
	...
	if index > MAX_PER_CATEGORY then
		warn(("[PLACEMENT] LÃ­mite alcanzado para categorÃ­a %s"):format(cat))
		return nil
	end
...
RequestPurchase.OnServerEvent:Connect(function(plr: Player, upgradeId: string)
	...
	if not def then
		warn(("[PURCHASE] Upgrade invÃ¡lido: %s"):format(upgradeId))
		return
	end
	...
	if not ok then
		...
		elseif reason == "REACH_CAP" then
			notifyPlayer(plr, "?? Max upgrades reached!", 2)
		end
		return
	end

	-- ? Actualizar Guardian para permitir compra legÃ­tima
	...
	-- ? Upgrade_8: Reinforced Walls (+20 HP max por compra, sin modelo fÃ­sico)
...
-------------------------------------------------------------------------
-- ?? SISTEMA DE COMPRA DE POWERUPS DESDE MÃQUINA
-------------------------------------------------------------------------
if PurchasePowerUp then
	PurchasePowerUp.OnServerEvent:Connect(function(plr: Player)
		...
		-- Intentar compra desde la mÃ¡quina
		...
		if success then
			...
			-- ?? Spawnear chicle/gumball fÃ­sico desde la mÃ¡quina
			...
			if vendingMachine and powerUpId then
				...
				if slot then
					-- Spawnear gumball en posiciÃ³n del slot + offset hacia arriba
			...
			if Config.DEBUG_MODE then
				print(("[POWERUP] ? %s comprÃ³ powerup '%s' desde mÃ¡quina"):format(plr.Name, powerUpId or "unknown"))
			end
		else
			...
			if Config.DEBUG_MODE then
				print(("[POWERUP] ? %s intentÃ³ comprar pero fallÃ³: %s"):format(plr.Name, errorMsg or "unknown"))
			end
		end
	end)
end
...
if UseFromInventory then
	UseFromInventory.OnServerEvent:Connect(function(plr: Player, slotIndex: number)
		-- Validar que slotIndex sea un nÃºmero vÃ¡lido
		...
		if success then
			...
			if Config.DEBUG_MODE then
				print(("[POWERUP] ? %s usÃ³ powerup desde slot %d"):format(plr.Name, slotIndex))
			end
		else
			notifyPlayer(plr, "? Slot empty or invalid", 2)

			if Config.DEBUG_MODE then
				print(("[POWERUP] ? %s intentÃ³ usar slot %d pero fallÃ³"):format(plr.Name, slotIndex))
			end
		end
	end)
end
...
-------------------------------------------------------------------------
-- SISTEMA DE REPARACIÃN (con streak escalable de EconomyModule)
-------------------------------------------------------------------------
...
RequestRepairPreview.OnServerInvoke = function(plr: Player)
	...
	local damageRatio = missing / max -- % de daÃ±o (0.0 - 1.0)
...
RequestRepair.OnServerEvent:Connect(function(plr: Player, amount: number)
	...
	if not checkRateLimit(plr.UserId, "Repair") then
		notifyPlayer(plr, "?? Repairing too fast!", 2)
		return
	end
	...
	-- ? Reset streak si llegÃ³ a HP mÃ¡ximo
	if Base.GetHP(plr.UserId) >= max then
		Economy.ResetRepairStreak(plr.UserId)
		if DEBUG then
			print(("[REPAIR] Streak reseteado - HP mÃ¡ximo alcanzado"):format())
		end
	end
...
RequestPrestige.OnServerEvent:Connect(function(plr: Player)
	if not Config.PRESTIGE.Enabled then
		notifyPlayer(plr, "?? Prestige not available yet!", 3)
		return
	end
	...
	if s.PrestigeLevel >= Config.PRESTIGE.MaxPrestige then
		notifyPlayer(plr, "?? Max prestige reached!", 3)
		return
	end

	-- Confirmar (esto deberÃ­a hacerse con un diÃ¡logo en cliente)
...
RequestDailyReward.OnServerEvent:Connect(function(plr: Player)
	...
	-- Determinar dÃ­a actual (reset si pasÃ³ mÃ¡s de 1 dÃ­a)
...
RequestLeaderboard.OnServerInvoke = function(plr: Player, leaderboardType: string?)
	-- AquÃ­ puedes implementar diferentes tipos de leaderboards
...
local Commands = {
	...
	meteors = function(plr: Player, args: {string})
		...
		Events:MeteorStorm()
		notifyPlayer(plr, string.format("?? Spawning %d meteors!", count), 3)
	end,
...
AdminCommand.OnServerEvent:Connect(function(plr: Player, commandString: string)
	if not checkRateLimit(plr.UserId, "Command") then
		notifyPlayer(plr, "?? Too many commands!", 2)
		return
	end
...
if not DashRequest then
	...
	warn("[Main.Server] RemoteEvent 'DashRequest' creado automÃ¡ticamente")
end

-- ? NUEVO: RemoteEvent para feedback de evasiÃ³n
...
if not DashEvaded then
	...
	warn("[Main.Server] RemoteEvent 'DashEvaded' creado automÃ¡ticamente")
end
...
local DASH_DURATION = 0.3 -- duraciÃ³n de invulnerabilidad
...
DashRequest.OnServerEvent:Connect(function(plr: Player, direction: Vector3?)
	...
	-- ? POWERUP: Verificar si SuperDash estÃ¡ activo (sin cooldown)
	...
	-- Validar direcciÃ³n (anti-exploit)
	...
	-- ? TELETRANSPORTE INSTANTÃNEO (20 studs)
	local originPos = humanoidRootPart.Position
	local targetPos = originPos + (direction * DASH_DISTANCE)

	-- Raycast para evitar teletransporte a travÃ©s de paredes
	...
	if rayResult then
		-- Si hay obstÃ¡culo, teleportar hasta justo antes del obstÃ¡culo
	...
	-- ? Efectos de partÃ­culas en origen
	...
	-- ? Efectos de partÃ­culas en destino
	...
	if Base and Base.SetInvulnerable then
		...
		warn("[DASH] ?? BaseModule no disponible - no se aplicÃ³ invulnerabilidad")
	end
end)
...
-------------------------------------------------------------------------
-- SISTEMA DE ECONOMÃA (LOOP PRINCIPAL)
-------------------------------------------------------------------------
task.spawn(function()
	while true do
...
if eventsEnabled then
	task.spawn(function()
		...
		while true do
			...
			-- Ajustar por nÃºmero de jugadores
			...
			print(("[WAVE] Meteoritos: %d | DaÃ±o: %d | Jugadores: %d"):format(
				waveConfig.Meteors, waveConfig.Damage, playerCount
				))
			print(("-----------------------------------------------------------"):rep(1))

			-- Anuncio
			broadcastNotification(string.format("?? WAVE %d INCOMING!", ServerState.CurrentWave), 5)
			...
			task.wait(0.3) -- Esperar solo 0.3s para feedback instantÃ¡neo

			if Config.DEBUG_MODE then
				print(("[WAVE] ? Wave %d completada"):format(ServerState.CurrentWave))
			end

			-- ? Incrementar contador Y enviar resultado ÃPICO personalizado
			for _, plr in ipairs(Players:GetPlayers()) do
				...
				if not diedDuringWave then
					...
					-- ? ENVIAR RESULTADO ÃPICO DE VICTORIA
					if WaveResult then
						local waveData = {
							result = "victory",
						...
						warn("[WAVE] ?? WaveResult remote no existe!")
					end
					...
					if Base.GetMeteorsSurvived(plr.UserId) == 100 and Achievements then
						...
						--[[
					?? SISTEMA DE POWERUPS AUTOMÃTICOS DESACTIVADO
					------------------------------------------------
					Los powerups ahora se obtienen comprando en la mÃ¡quina expendedora.
					El cÃ³digo antiguo estÃ¡ comentado abajo por si se necesita restaurar:
					...
					-- Spawnear powerup solo si cumple la condiciÃ³n
					if shouldSpawnPowerUp then
						-- ? Pasar el nÃºmero de wave para el sistema hÃ­brido
					...
					if Config.DEBUG_MODE then
						print(("[WAVE] ? Jugador %s SOBREVIVIÃ wave %d"):format(plr.Name, ServerState.CurrentWave))
					end
				else
					-- ? Jugador MURIÃ
					-- ? ENVIAR RESULTADO ÃPICO DE DERROTA
					if WaveResult then
						local waveData = {
							result = "defeat",
							waveNumber = ServerState.CurrentWave,
					...
					if Config.DEBUG_MODE then
						print(("[WAVE] ? Jugador %s MURIÃ durante wave %d"):format(plr.Name, ServerState.CurrentWave))
					end
				end
			end

			-- ? INCREMENTAR WAVE DESPUÃS de enviar mensajes
			...
			if def and def.ModelName then
				...
				for i = 1, owned do
					if totalRebuilt >= Config.MAX_MODELS_PER_BASE then
						warn(("[REBUILD] LÃ­mite alcanzado para %s"):format(plr.Name))
						return
					end
...
-------------------------------------------------------------------------
-- ?? INICIALIZAR AMBIENTE APOCALÃPTICO
-------------------------------------------------------------------------
EnvironmentManager:Initialize()

if Config.DEBUG_MODE then
	print("[MAIN] ? Ambiente apocalÃ­ptico inicializado")
end
...
local function diagnosticBillboards(userId: number)
	task.delay(2, function() -- Esperar 2 segundos a que se creen las bases
		...
		if billboardCount > 1 then
			warn(("[DIAGNOSTIC] ?? PROBLEMA: Hay %d billboards (deberÃ­a ser 1)"):format(billboardCount))
		end
	end)
end
-------------------------------------------------------------------------
...
Players.PlayerAdded:Connect(function(plr: Player)
	...
	if elapsed > 0 and lastOut > 0 then
		...
		if s and s.IncomePerSec > 0 then
			...
			if DEBUG then
				print(("[OFFLINE] %s ganÃ³ $%d en %ds"):format(plr.Name, earned, elapsed))
			end
		end
	end
...
print("-----------------------------------------------------------------------")
print(("[CONFIG] VersiÃ³n: 5.0 PRODUCTION"):format())
...
if not isValid then
	error(("[CONFIG] ? ConfiguraciÃ³n invÃ¡lida: %s"):format(errorMsg or "unknown"))
end
