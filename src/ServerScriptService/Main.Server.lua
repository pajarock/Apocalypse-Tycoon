...
if eventsEnabled then
	task.spawn(function()
		...
		while true do
			...
			-- ✅ BOSS SYSTEM: Diferenciación entre Mini-Boss y Boss Completo
			if ServerState.CurrentWave % 10 == 0 then
				-- 💀 BOSS COMPLETO (Wave 10, 20, 30...)
				task.wait(5)
				Events:BossMeteor(ServerState.CurrentWave, true) -- 4 fases
				task.wait(30)  -- Boss completo tarda más (4 fases)
			elseif ServerState.CurrentWave % 5 == 0 then
				-- 💀 MINI-BOSS (Wave 5, 15, 25...)
				task.wait(5)
				Events:BossMeteor(ServerState.CurrentWave, false) -- 1 fase aleatoria
				task.wait(15)  -- Mini-boss tarda menos (1 fase)
			else
				-- Wave normal
			...
			for _, plr in ipairs(Players:GetPlayers()) do
				...
				if not diedDuringWave then
					...
					if s then
						...
						if isBossWave then
							-- 💀 BOSS COMPLETO: $500 × (wave/10) + 100% HP
							reward = 500 * (ServerState.CurrentWave / 10)
							healAmount = Config.BASE_MAX_HP - Base.GetHP(plr.UserId) -- Curar 100%

							if ShowNotification then
								ShowNotification:FireClient(plr, string.format("💀 BOSS DEFEATED! +$%d + Full HP!", reward), 5)
							end
						elseif isMiniBossWave then
							-- 💀 MINI-BOSS: $100 × wave + 100% HP
							reward = 100 * ServerState.CurrentWave
							healAmount = Config.BASE_MAX_HP - Base.GetHP(plr.UserId) -- Curar 100%

							if ShowNotification then
								ShowNotification:FireClient(plr, string.format("💀 MINI-BOSS DEFEATED! +$%d + Full HP!", reward), 4)
							end
						else
							-- ✅ WAVE NORMAL: $50 × wave
							reward = 50 * ServerState.CurrentWave

							if ShowNotification then
								ShowNotification:FireClient(plr, string.format("✅ Wave %d Survived! Bonus: $%d", ServerState.CurrentWave, reward), 3)
							end
						end
						...
						if reward > 0 then
							...
							-- Actualizar Guardian si está activo
							Base.UpdateGuardianTarget(plr.UserId, s.Cash)

							if Config.DEBUG_MODE then
								print(("[WAVE] 💰 %s recibió $%d por sobrevivir wave %d"):format(plr.Name, reward, ServerState.CurrentWave))
							end
						end

						-- Curar HP (Boss/Mini-Boss)
						if healAmount > 0 then
							Base.AddHP(plr.UserId, healAmount)
							...
							if Config.DEBUG_MODE then
								print(("[WAVE] 💚 %s curado +%d HP"):format(plr.Name, healAmount))
							end
						end
					end
...
--═══════════════════════════════════════════════════════════════════════
-- 🏃 DASH SYSTEM - Sistema de evasión con invulnerabilidad
--═══════════════════════════════════════════════════════════════════════

-- Crear RemoteEvent para Dash
...
local DashCooldowns = {} -- [userId] = tick()

-- Configuración del dash
local DASH_CONFIG = {
	Cooldown = 4, -- segundos
	InvulnerabilityDuration = 0.3, -- segundos
	DashDistance = 25, -- studs
	DashDuration = 0.15, -- segundos (rápido)
}
...
DashRequest.OnServerEvent:Connect(function(plr: Player, direction: Vector3?)
	...
	-- Verificar cooldown
	local now = tick()
	local lastDash = DashCooldowns[userId] or 0

	if now - lastDash < DASH_CONFIG.Cooldown then
		-- Enviar notificación de cooldown al cliente
		if ShowNotification then
			local remaining = math.ceil(DASH_CONFIG.Cooldown - (now - lastDash))
			ShowNotification:FireClient(plr, string.format("⏳ Dash ready in %ds", remaining), 1)
		end
		return
	end
	...
	if not direction or typeof(direction) ~= "Vector3" then
		warn(("[DASH] Dirección inválida de %s"):format(plr.Name))
		return
	end
	...
	if direction.Magnitude > 0 then
		...
		warn(("[DASH] Dirección zero de %s"):format(plr.Name))
		return
	end
	...
	Base.SetInvulnerable(userId, true, DASH_CONFIG.InvulnerabilityDuration)
	...
	if char then
		...
		if rootPart and humanoid and humanoid.Health > 0 then
			...
			dashVelocity.MaxForce = Vector3.new(1e6, 0, 1e6) -- No afectar Y (gravedad)
			dashVelocity.Velocity = direction * (DASH_CONFIG.DashDistance / DASH_CONFIG.DashDuration)
			dashVelocity.Parent = rootPart

			-- Remover después del dash
			Debris:AddItem(dashVelocity, DASH_CONFIG.DashDuration)

			-- Efectos visuales (enviados al cliente a través de remotes)
			if ScreenFlash then
				ScreenFlash:FireClient(plr, "White", 0.3, DASH_CONFIG.InvulnerabilityDuration)
			end

			if DEBUG then
				print(("[DASH] %s dasheó en dirección %s"):format(plr.Name, tostring(direction)))
			end
		end
	end
...
if DEBUG then
	print("[DASH] ✅ Sistema de dash iniciado")
end
