...
--[[
	...
	Sistema de inventario para powerups comprados en la mÃ¡quina expendedora.
...
-- -----------------------------------------------------------------------
-- POWERUP DISPLAY CONFIG (solo info de UI, no lÃ³gica de juego)
-- -----------------------------------------------------------------------
...
local function createInventoryGUI(): ScreenGui
	...
	container.Rotation = -1 -- Leve inclinaciÃ³n urbana
	container.Parent = screenGui

	-- UICorner mÃ¡s redondeado
	...
	for i = 1, 5 do
		...
		slot.Rotation = math.random(-3, 3) -- RotaciÃ³n random para look urbano
		...
		-- Icon Label (emoji del powerup) - MÃS GRANDE
		...
		-- ?? Click handler con ANIMACIÃN ÃPICA
	...
	for i = 1, 5 do
		...
		if slot then
			...
			if powerUpId and PowerUpDisplayInfo[powerUpId] then
				...
				-- ?? ANIMACIÃN ÃPICA SI ES NUEVO ITEM
				if isNewItem then
					-- Bounce Ã©pico
					local originalSize = slot.Size
...
-- Inicializar con inventario vacÃ­o
updateSlots({})
