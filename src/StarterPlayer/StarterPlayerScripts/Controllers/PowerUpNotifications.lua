...
--[[
	...
	Sistema de notificaciones visuales ÃÂPICAS para power-ups.

	FEATURES:
	? Banner grande cuando colectas power-up
	? Floating income text cuando Coin Rain activo
	? Screen flash de colecciÃÂ³n
	? Sound effects impactantes
	? Animaciones fluidas

	UBICACIÃÂN: StarterPlayerScripts/Controllers/PowerUpNotifications
]]
...
local CONFIG = {
	PowerUps = {
		ShieldBubble = {
			...
			Message = "ÃÂ¡INVULNERABLE POR 10s!",
		},
		SlowMotion = {
			Icon = "??",
			Name = "SLOW MOTION",
			Color = Color3.fromRGB(150, 50, 200),
			Message = "ÃÂ¡Meteoritos 50% mÃÂ¡s lentos!",
		},
		CoinRain = {
			Icon = "??",
			Name = "COIN RAIN",
			Color = Color3.fromRGB(255, 215, 0),
			Message = "ÃÂ¡INGRESOS x2 POR 30s!",
		},
		AutoRepair = {
			Icon = "??",
			Name = "AUTO-REPAIR",
			Color = Color3.fromRGB(0, 255, 100),
			Message = "ÃÂ¡Regenerando +5 HP/s!",
		},
		DamageBoost = {
			Icon = "??",
			Name = "DAMAGE BOOST",
			Color = Color3.fromRGB(255, 50, 50),
			Message = "ÃÂ¡DAÃÂO x3 POR 15s!",
		},
	},
...
local function monitorCoinRain()
	RunService.Heartbeat:Connect(function()
		...
		-- Si incrementÃÂ³, mostrar floating text
...
local function onPowerUpCollected(powerUpType: string, duration: number)
	...
	sound.SoundId = "rbxassetid://6026984224" -- Collect sound ÃÂ©pico
	sound.Volume = 0.5
