...
--[[
	...
	Maneja efectos de cÃ¡mara como shake y flash para feedback visual.

	FEATURES:
	? Camera shake con intensidades configurables
	? Screen flash con colores custom
	? Mobile-safe (reduce intensidad automÃ¡ticamente)
	? Settings toggle (respeta preferencias del jugador)
	? Thread-safe (mÃºltiples shakes pueden ocurrir simultÃ¡neamente)
	...
	- Mobile automÃ¡ticamente reduce intensidad 50%
]]
...
local MOBILE_REDUCTION = 1 -- 50% menos intenso en mobile
...
function CameraController:Shake(intensity: string, duration: number)
	...
	-- si ya tienes un RenderStepped, asegÃºrate de que se conecte
	...
	if not intensityValue then
		warn(("[CameraController] Intensidad invÃ¡lida: %s"):format(intensity))
		return
	end
...
--[[
	...
	@param intensity number - Opacidad mÃ¡xima (0.0 - 1.0)
	@param duration number - DuraciÃ³n en segundos
]]
...
--[[
	Habilita/deshabilita efectos de cÃ¡mara.
...
--[[
	Retorna si los efectos estÃ¡n habilitados.
...
print("[CameraController] ? MÃ³dulo cargado")
