...
local function createBaseShieldEffect(): {Instance}
	...
	-- (Esto irÃ­a en la base fÃ­sica, pero podemos hacer un indicador visual)
...
local function createFullHealEffect(): {Instance}
	...
	-- Burst de partÃ­culas verdes curativas
	...
	-- Desactivar despuÃ©s del burst inicial
...
local function createMeteorJammerEffect(): {Instance}
	...
	-- ?? PULSOS ELECTROMAGNÃTICOS EXPANDIÃNDOSE (sin burbuja)
	-- Crear mÃºltiples anillos que se expanden desde el jugador
	...
	-- PartÃ­culas elÃ©ctricas moradas
	local particles = Instance.new("ParticleEmitter")
	particles.Texture = "rbxasset://textures/particles/sparkles_main.dds"
...
local function createUltraChargeEffect(): {Instance}
	...
	-- ? EXPLOSIÃN MASIVA DE RAYOS ELÃCTRICOS
	...
	-- MÃºltiples anillos de energÃ­a girando
	local rings = {}
	for i = 1, 3 do
		local ring = Instance.new("Part")
...
local function createEggCatalystEffect(): {Instance}
	...
	-- ?? EXPLOSIÃN DORADA MÃGICA
	local sparkles = Instance.new("Sparkles")
	sparkles.SparkleColor = Color3.fromRGB(255, 220, 100)
	sparkles.Parent = humanoidRootPart

	-- PartÃ­culas masivas doradas
...
-- -----------------------------------------------------------------------
-- MENSAJES ÃPICOS DE ACTIVACIÃN
-- -----------------------------------------------------------------------
...
local function showEpicPowerUpMessage(powerUpId: string, color: Color3)
	...
	mainText.Rotation = -5 -- InclinaciÃ³n urbana
	mainText.Parent = gui
...
PowerUpActivated.OnClientEvent:Connect(function(powerUpId: string, duration: number)
	print(("[PowerUpUI] ?? Activando powerup: %s (duraciÃ³n: %.1fs)"):format(powerUpId, duration or 0))

	-- ? LIMPIAR powerup existente si ya estÃ¡ activo
	...
	-- ?? MOSTRAR MENSAJE ÃPICO ESTILO GRAFITI
	task.spawn(function()
...
PowerUpExpired.OnClientEvent:Connect(function(powerUpId: string)
	...
	if ActivePowerUpFrames[powerUpId] then
		...
		print(("[PowerUpUI] ?? No se encontrÃ³ frame activo para: %s"):format(powerUpId))
	end
...
-- Powerup recogido (feedback instantÃ¡neo)
PowerUpCollected.OnClientEvent:Connect(function(powerUpId: string)
	-- Sonido de pickup (puedes agregar un SoundService aquÃ­)
