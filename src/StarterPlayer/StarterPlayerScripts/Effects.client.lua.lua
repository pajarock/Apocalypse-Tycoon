...
CashTick.OnClientEvent:Connect(function(gain: number)
	if typeof(gain) == "number" and gain ~= 0 then  -- ✅ Aceptar tanto positivos como negativos
		makeCashPopup(gain)
	end
end)
--!strict
-- ShieldVFX.client.lua  (StarterPlayerScripts)
-- Muestra un Highlight en la base seg�n ShieldLevel del jugador
...
local function applyShieldVFX(level: number)
    ...
    -- peque�o �pulso� al cambiar
    highlight.FillTransparency = 0.9
    TweenService:Create(highlight, TweenInfo.new(0.25), { FillTransparency = 0.7 }):Play()
end

-- Cuando aparezca el character, aseg�rate de crear el efecto
