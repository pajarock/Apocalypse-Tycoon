...
-- Label de nÃºmeros en LuckiestGuy (GRANDE)
...
label.ZIndex = 3
label.Rotation = -2 -- InclinaciÃ³n grafiti
...
-- ? FIX: Definir funciÃ³n update() ANTES de usarla
...
local function updateRepairButton()
	...
	if not preview.canRepair or preview.willHeal <= 0 then
		...
		if preview.streak > 0 then
			streakText = string.format(" (x%d)", preview.streak + 1) -- +1 porque serÃ¡ el prÃ³ximo
		end
...
-- ? FIX: Click en botÃ³n de reparaciÃ³n
...
task.spawn(function()
	while true do
		update()
		updateRepairButton() -- ? FIX: Actualizar botÃ³n de reparaciÃ³n cada 2 segundos
		task.wait(2)
	end
end)

update()
updateRepairButton() -- ? FIX: Actualizar botÃ³n al inicio