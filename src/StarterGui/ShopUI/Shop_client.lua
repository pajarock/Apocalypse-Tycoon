...
-- Panel principal con estilo urbano (tamaÃÂ±o original)
local frame = Instance.new("Frame")
frame.Name = "ShopFrame"
frame.AnchorPoint = Vector2.new(0, 0)
frame.Position = UDim2.new(0, 20, 0, 100)
frame.Size = UDim2.fromOffset(280, 360) -- ? TamaÃÂ±o fijo que no desborda
frame.BackgroundColor3 = Color3.fromRGB(15, 15, 18)
frame.BackgroundTransparency = 0.05
frame.BorderSizePixel = 0
frame.Rotation = -1 -- RotaciÃÂ³n sutil urbano
frame.Parent = gui
...
title.Rotation = -2 -- InclinaciÃÂ³n grafiti
title.ZIndex = 2
title.Parent = frame

-- ? PESTAÃÂAS (TABS) - Contenedor horizontal
...
list.Position = UDim2.fromOffset(10, 76) -- Debajo de pestaÃÂ±as
...
local shieldLevel = 0
local currentTab = "Income" -- ? PestaÃÂ±a activa por defecto

-- FunciÃÂ³n para obtener estado del servidor
...
-- ? NUEVO: Upgrades organizados por categorÃÂ­a con 4 pestaÃÂ±as
local CATEGORIES = {
	...
	{ name = "Eggs", emoji = "??", upgrades = {} }, -- ? 4ta pestaÃÂ±a para gacha (vacÃÂ­a por ahora)
}

-- ? Crear botÃÂ³n de pestaÃÂ±a
...
local function refreshAll()
	...
	-- ? FIX: Mostrar reducciÃÂ³n de daÃÂ±o del shield actual (18% por nivel - BALANCEADO)
	...
	-- ? Actualizar estilo de pestaÃÂ±as (activa vs inactiva)
	...
	-- ? Mostrar solo upgrades de la categorÃÂ­a activa
	local layoutOrder = 0
	for _, category in ipairs(CATEGORIES) do
		if category.name == currentTab then
			...
			-- Ocultar botones de otras categorÃÂ­as
			for _, upgId in ipairs(category.upgrades) do
