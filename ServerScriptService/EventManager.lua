...
--[[
	BOSS METEOR - Sistema de jefes cada 5 waves

	Mini-Boss (Wave 5, 15, 25...): 1 fase aleatoria
	Boss Completo (Wave 10, 20, 30...): 4 fases en orden

	FASES:
	1. "Esquinas del Caos" - 4 meteoritos en esquinas
	2. "Círculo Infernal" - 6 meteoritos en círculo
	3. "El Coloso" - 1 meteorito GIGANTE
	4. "Triángulo de Fuego" - 3 meteoritos en triángulo
]]

local BOSS_PHASES = {
	{
		Name = "Esquinas del Caos",
		Description = "4 meteoritos en las esquinas",
		Icon = "🔥",
		Meteors = {
			{Type = "Small", Delay = 0, Position = "NW"},
			{Type = "Small", Delay = 1, Position = "NE"},
			{Type = "Small", Delay = 2, Position = "SE"},
			{Type = "Small", Delay = 3, Position = "SW"},
		}
	},
	{
		Name = "Círculo Infernal",
		Description = "6 meteoritos en círculo rotando",
		Icon = "⭕",
		Meteors = {
			{Type = "Normal", Delay = 0, Angle = 0},
			{Type = "Normal", Delay = 0.5, Angle = 60},
			{Type = "Normal", Delay = 1, Angle = 120},
			{Type = "Normal", Delay = 1.5, Angle = 180},
			{Type = "Normal", Delay = 2, Angle = 240},
			{Type = "Normal", Delay = 2.5, Angle = 300},
		}
	},
	{
		Name = "El Coloso",
		Description = "Un meteorito gigante y lento",
		Icon = "☄️",
		Meteors = {
			{Type = "Large", Delay = 0, Position = "Center", FallTime = 8},
		}
	},
	{
		Name = "Triángulo de Fuego",
		Description = "3 meteoritos grandes simultáneos",
		Icon = "🔺",
		Meteors = {
			{Type = "Large", Delay = 0, Angle = 0},
			{Type = "Large", Delay = 0, Angle = 120},
			{Type = "Large", Delay = 0, Angle = 240},
		}
	}
}

local function getPositionOffset(pattern: string, radius: number): Vector3
	local offsets = {
		NW = Vector3.new(-radius, 0, radius),
		NE = Vector3.new(radius, 0, radius),
		SE = Vector3.new(radius, 0, -radius),
		SW = Vector3.new(-radius, 0, -radius),
		Center = Vector3.new(0, 0, 0),
	}
	return offsets[pattern] or Vector3.new(0, 0, 0)
end

local function getAngleOffset(angle: number, radius: number): Vector3
...
function EventManager:BossMeteor(waveNum: number, isFullBoss: boolean)
	waveNum = waveNum or 5
	isFullBoss = isFullBoss or false

	if Config.DEBUG_MODE then
		print(("[BOSS] Iniciando Boss Meteor - Wave %d | Full Boss: %s"):format(
			waveNum, tostring(isFullBoss)
		))
	end
	...
	if isFullBoss then
		...
		phasesToRun = {randomPhase}

		local phase = BOSS_PHASES[randomPhase]
		-- Anuncio épico
		if ShowNotification then
			for _, plr in ipairs(Players:GetPlayers()) do
				ShowNotification:FireClient(plr, string.format("💀 MINI-BOSS: %s", phase.Name), 4)
			end
		end
	end

	-- Ejecutar cada fase
	for _, phaseIndex in ipairs(phasesToRun) do
		local phase = BOSS_PHASES[phaseIndex]

		if Config.DEBUG_MODE then
			print(("[BOSS] Ejecutando fase %d: %s"):format(phaseIndex, phase.Name))
		end

		-- Anuncio de fase (si es boss completo)
		if isFullBoss and ShowNotification then
			for _, plr in ipairs(Players:GetPlayers()) do
				ShowNotification:FireClient(plr, string.format("%s FASE %d: %s", phase.Icon, phaseIndex, phase.Name), 3)
			end
		end

		task.wait(2) -- Esperar antes de lanzar meteoritos

		-- Lanzar meteoritos de la fase
		for _, plr in ipairs(Players:GetPlayers()) do
			local base = getPlayerBasePart(plr)
			if not base then continue end

			for _, meteorData in ipairs(phase.Meteors) do
		...
		-- Esperar a que terminen los meteoritos
		local phaseDuration = 6
		if phase.Name == "El Coloso" then
			phaseDuration = 10 -- El coloso tarda más
		end
		task.wait(phaseDuration)
	end

	-- Recompensa al final (implementar en Main.Server)
	if Config.DEBUG_MODE then
		print(("[BOSS] Boss Meteor completado - Wave %d"):format(waveNum))
	end
end
